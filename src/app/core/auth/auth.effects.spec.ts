...
describe('AuthEffects', () => {
  ...
  describe('login$', () => {
    ...
    it('should dispatch loginFailure when login throws an error', async () => {
      ...
      const result = await firstValueFrom(effects.login$);
      expect(result).toEqual(AuthActions.loginFailure({ error: 'Network error' }));
    });
    ...
    it('should dispatch loginFailure when response has no user', async () => {
      ...
      const result = await firstValueFrom(effects.login$);
      expect(result).toEqual(AuthActions.loginFailure({ error: 'No user returned' }));
    });
  });
  ...
  describe('loginSuccess$', () => {
    it('should save token and user to localStorage', async () => {
      ...
      actions$ = of(
        AuthActions.loginSuccess({ user: testUser, token: 'access-token', redirect: false }),
      );
    ...
    it('should navigate to returnUrl when redirect is true', async () => {
      ...
      actions$ = of(
        AuthActions.loginSuccess({ user: testUser, token: 'access-token', redirect: true }),
      );
