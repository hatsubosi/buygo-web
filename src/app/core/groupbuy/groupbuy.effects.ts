...
import { ListGroupBuysResponse, GetGroupBuyResponse, CreateOrderResponse, CreateOrderItem, CreateGroupBuyResponse, UpdateGroupBuyResponse, AddProductResponse, RoundingConfig, RoundingMethod, GetMyOrdersResponse, UpdateOrderResponse, UpdatePaymentInfoResponse } from '../api/api/v1/groupbuy_pb';
...
export class GroupBuyEffects {
    ...
    loadGroupBuys$ = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(() =>
                this.client.listGroupBuys({}).then(
                    (res: ListGroupBuysResponse) => GroupBuyActions.loadGroupBuysSuccess({ groupBuys: res.groupBuys })
    ...
    loadGroupBuyDetail$ = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(({ id }) =>
                this.client.getGroupBuy({ groupBuyId: id }).then(
                    (res: GetGroupBuyResponse) => {
                        if (!res.groupBuy) throw new Error('Project not found');
                        return GroupBuyActions.loadGroupBuyDetailSuccess({ groupBuy: res.groupBuy, products: res.products });
                    }
    ...
    submitOrder$ = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(({ groupBuyId, contactInfo, shippingAddress, items }) => {
                const orderItems = items.map((item: any) => new CreateOrderItem({
                ...
                return this.client.createOrder({
                    ...
                    (res: CreateOrderResponse) => GroupBuyActions.submitOrderSuccess({ orderId: res.orderId })
        ...
        this.actions$.pipe(
            ...
            switchMap(() =>
                this.client.getMyOrders({}).then(
                    (res: GetMyOrdersResponse) => GroupBuyActions.loadMyOrdersSuccess({ orders: res.orders })
