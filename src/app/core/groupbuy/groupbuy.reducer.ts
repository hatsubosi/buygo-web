...
export const groupBuyReducer = createReducer(
    ...
    on(GroupBuyActions.loadGroupBuysFailure, (state, { error }) => ({ ...state, loadingList: false, listError: error })),
    ...
    on(GroupBuyActions.loadGroupBuyDetailFailure, (state, { error }) => ({ ...state, loadingDetail: false, detailError: error })),
    ...
    // Submit Order
    on(GroupBuyActions.submitOrder, (state) => ({ ...state, submittingOrder: true, submitOrderError: null, lastCreatedOrderId: null })),
    on(GroupBuyActions.submitOrderSuccess, (state, { orderId }) => ({ ...state, submittingOrder: false, cart: [], lastCreatedOrderId: orderId })), // Clear cart on success
    on(GroupBuyActions.submitOrderFailure, (state, { error }) => ({ ...state, submittingOrder: false, submitOrderError: error })),

    // My Orders
    on(GroupBuyActions.loadMyOrders, (state) => ({ ...state, loadingMyOrders: true, myOrdersError: null })),
    on(GroupBuyActions.loadMyOrdersSuccess, (state, { orders }) => ({ ...state, loadingMyOrders: false, myOrders: orders })),
    on(GroupBuyActions.loadMyOrdersFailure, (state, { error }) => ({ ...state, loadingMyOrders: false, myOrdersError: error })),

    // Update User Order / Payment
    on(GroupBuyActions.updateUserOrder, (state) => ({ ...state, updatingOrder: true, updateOrderError: null })),
    ...
    on(GroupBuyActions.updateUserOrderFailure, (state, { error }) => ({ ...state, updatingOrder: false, updateOrderError: error })),

    on(GroupBuyActions.updatePaymentInfo, (state) => ({ ...state, updatingOrder: true, updateOrderError: null })), // Re-use updatingOrder state or create separate? Re-use is fine.
    on(GroupBuyActions.updatePaymentInfoSuccess, (state, { order }) => ({
        ...state,
        updatingOrder: false,
        myOrders: state.myOrders.map(o => o.id === order.id ? order : o)
    })),
    on(GroupBuyActions.updatePaymentInfoFailure, (state, { error }) => ({ ...state, updatingOrder: false, updateOrderError: error })),


    // Create/Update Actions
    on(GroupBuyActions.createGroupBuy, (state) => ({ ...state, actionLoading: true, actionError: null })),
    on(GroupBuyActions.createGroupBuySuccess, (state, { groupBuy }) =>
        adapter.addOne(groupBuy, { ...state, actionLoading: false })
    ),
    on(GroupBuyActions.createGroupBuyFailure, (state, { error }) => ({ ...state, actionLoading: false, actionError: error })),

    on(GroupBuyActions.updateGroupBuy, (state) => ({ ...state, actionLoading: true, actionError: null })),
    on(GroupBuyActions.updateGroupBuySuccess, (state, { groupBuy }) =>
        adapter.upsertOne(groupBuy, { ...state, actionLoading: false })
    ),
    on(GroupBuyActions.updateGroupBuyFailure, (state, { error }) => ({ ...state, actionLoading: false, actionError: error })),
    ...
    on(GroupBuyActions.addProductFailure, (state, { error }) => ({ ...state, actionLoading: false, actionError: error })),
);
