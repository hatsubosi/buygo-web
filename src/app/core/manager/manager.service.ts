...
import { Order, PaymentStatus } from '../api/api/v1/project_pb';
...
export class ManagerService {
    ...
    async loadProjectOrders(projectId: string) {
        this.isLoading.set(true);
        this.error.set(null);
        try {
            const res = await this.client.listProjectOrders({ projectId });
    ...
    async batchUpdateStatus(projectId: string, specId: string, targetStatus: number, count: number) {
        try {
