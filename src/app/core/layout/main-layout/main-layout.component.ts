...
@Component({
    ...
    <div class="min-h-screen bg-[#0a0a0a] text-white selection:bg-blue-500/30">
        <!-- Navbar -->
        <nav class="fixed top-0 left-0 right-0 z-50 border-b border-white/5 bg-[#0a0a0a]/80 backdrop-blur-xl transition-all duration-300">
            <div class="relative mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
                <!-- Logo -->
                <a routerLink="/" class="flex items-center gap-2">
                    <span class="text-2xl font-bold tracking-tight bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent">
                        BuyGo
                    </span>
                </a>
                ...
                <div class="flex items-center gap-4">
                    @if (auth.isAuthenticated()) {
                        <div class="hidden md:flex items-center gap-2">
                           <div class="h-8 w-8 rounded-full bg-gradient-to-tr from-blue-500 to-purple-500 flex items-center justify-center text-xs font-bold">
                        ...
                        <a [routerLink]="['/login']" [queryParams]="{returnUrl: router.url}" class="hidden sm:block">
                     ...
                     <!-- Mobile Menu Toggle -->
                    <button class="md:hidden p-2 -mr-2 text-gray-400 hover:text-white transition-colors" (click)="toggleMobileMenu()">
        ...
        <!-- Mobile Menu Overlay -->
        @if (mobileMenuOpen()) {
            <div class="fixed inset-0 top-16 z-40 bg-[#0a0a0a]/95 backdrop-blur-2xl border-t border-white/5 md:hidden flex flex-col animate-in fade-in slide-in-from-top-4 duration-200">
                <div class="flex flex-col p-6 space-y-6">
                    ...
                    @if (auth.isAuthenticated()) {
                        ...
                        <div class="flex items-center gap-3 mb-4">
                           <div class="h-10 w-10 rounded-full bg-gradient-to-tr from-blue-500 to-purple-500 flex items-center justify-center text-sm font-bold">
                                {{ auth.user()?.name?.charAt(0) || 'U' }}
                           </div>
                         ...
                         @if (canManage()) {
                             <a routerLink="/manager" (click)="closeMobileMenu()" class="flex items-center gap-3 text-lg text-gray-300 hover:text-white">
                                <span class="material-icons">dashboard</span> Manager Dashboard
                             </a>
                        }

                        <a routerLink="/user" (click)="closeMobileMenu()" class="flex items-center gap-3 text-lg text-gray-300 hover:text-white">
                            <span class="material-icons">person</span> Dashboard
                        </a>

                        <button (click)="logout()" class="flex items-center gap-3 text-lg text-red-400 hover:text-red-300 text-left w-full mt-4">
                            <span class="material-icons">logout</span> Sign Out
                        </button>
                    } @else {
                        <div class="h-px bg-white/10 my-4"></div>
                        <a [routerLink]="['/login']" (click)="closeMobileMenu()" [queryParams]="{returnUrl: router.url}" class="w-full">
