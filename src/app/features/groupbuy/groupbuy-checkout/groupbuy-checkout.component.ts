...
export class GroupBuyCheckoutComponent implements OnInit {
  ...
  getShippingTypeLabel(type: number): string {
    switch (type) {
      case ShippingType.MEETUP:
        return 'Meetup';
      case ShippingType.DELIVERY:
        return 'Delivery';
      case ShippingType.STORE_PICKUP:
        return 'Store Pickup';
      default:
        return 'Standard';
    }
  ...
  addressPlaceholder(): string {
    ...
    if (this.selectedConfig?.type === ShippingType.STORE_PICKUP)
      return 'e.g. 7-11 Ximen Store (123456)';
  ...
  submitOrder() {
    ...
    this.groupBuyService.submitOrder(
      projectId,
      this.contactInfo,
      this.shippingAddress,
      this.groupBuyService.cart(),
