        ...
        <div class="px-4 py-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @for (event of eventService.events(); track event.id) {
                    <app-ui-container customClass="h-full flex flex-col p-0 cursor-pointer transition-transform hover:scale-[1.02]" [routerLink]="['/event', event.id]">
                        ...
                        <div class="flex flex-1 flex-col p-6">
                            ...
                            <p class="text-sm text-gray-400 mb-4">
                                {{ toDate(event.startTime) | date:'mediumDate' }} - {{ toDate(event.endTime) | date:'mediumDate' }}
                            </p>
