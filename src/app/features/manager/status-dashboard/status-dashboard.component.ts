    ...
    <div class="min-h-screen pt-8 pb-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <app-ui-container customClass="p-8">
        ...
        <div class="mb-8 flex items-center justify-between">
          <div>
            <div class="mb-2 flex items-center gap-2">
              <a
                [routerLink]="['/manager/groupbuy', id()]"
            ...
            <tbody class="divide-y divide-white/5">
              @for (row of matrix(); track row.productId + row.specId) {
                <tr class="hover:bg-white/5 transition-colors">
                  <td
                    class="px-4 py-3 font-medium text-white sticky left-0 bg-[#0a0a0a] z-10 border-r border-white/5"
                  >
                  ...
                  <td
                    class="px-4 py-3 text-center border-r border-white/5 cursor-pointer hover:bg-white/10"
                    (click)="openDialog(row, 2, 1)"
                  >
                    <span class="block font-bold" [class.text-white]="row.cells[1].count > 0">{{
                      row.cells[1].count
                    }}</span>
                  </td>
                  ...
                  >
                    <span class="block font-bold" [class.text-blue-400]="row.cells[2].count > 0">{{
                      row.cells[2].count
                    }}</span>
                  </td>
                  ...
                  >
                    <span
                      ...
                      [class.text-indigo-400]="row.cells[3].count > 0"
                      >{{ row.cells[3].count }}</span
                    >
                  </td>
                  ...
                  >
                    <span
                      class="block font-bold"
                      [class.text-purple-400]="row.cells[4].count > 0"
                      >{{ row.cells[4].count }}</span
                    >
                  </td>

                  <!-- Ready (5) -> Sent (6) (DISABLED for Batch) -->
                  <td
                    class="px-4 py-3 text-center border-r border-white/5 cursor-not-allowed bg-white/5"
                    title="Ship individual orders in Order List"
                  >
                    <span class="block font-bold" [class.text-green-400]="row.cells[5].count > 0">{{
                      row.cells[5].count
                    }}</span>
                  </td>
                  ...
                  <td class="px-4 py-3 text-center">
                    <span
                      class="block font-bold"
                      [class.text-green-600]="row.cells[6].count > 0"
                      [class.text-gray-500]="row.cells[6].count === 0"
                      >{{ row.cells[6].count }}</span
                    >
                  </td>
                </tr>
              }
        ...
        @if (selectedAction) {
          ...
          >
            <div
              class="w-full max-w-md bg-[#1a1a1a] border border-white/10 rounded-xl p-6 shadow-2xl animate-scale-in"
            ...
            >
              ...
              <p class="text-gray-300 mb-6">
                ...
                <span class="font-bold text-white">{{
                  getStatusLabel(selectedAction!.fromStatus)
                }}</span>
                to
                <span class="font-bold text-white">{{
                  getStatusLabel(selectedAction!.targetStatus)
                }}</span>
              </p>

              <div class="mb-6">
                <label class="block text-sm text-gray-400 mb-2"
                  >Quantity (Max {{ selectedAction!.maxCount }})</label
                >
                <input
                  type="number"
                  [(ngModel)]="moveCount"
                  min="1"
                  [max]="selectedAction!.maxCount"
