        ...
        <div class="rounded-xl border border-white/10 bg-white/5 overflow-hidden">
          <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-white/5">
              @if (users().length === 0) {
                <tr>
                  <td colspan="4" class="px-6 py-8 text-center text-gray-500">Loading users...</td>
                </tr>
              } @else {
                  ...
                  <tr class="hover:bg-white/5">
                    ...
                    <td class="px-6 py-4">
                      <div class="flex gap-2">
                        @if (user.role !== 3) {
                          <!-- Cannot edit SysAdmin for safety for now -->
                          @if (user.role === 1) {
...
export class ManagerUserListComponent {
  ...
  page = signal(1);
  pageSize = signal(20);
  tokenStack = signal<string[]>(['']);
  currentPageToken = signal('');
  ...
  async loadUsers(pageToken: string = this.currentPageToken()) {
    this.isLoading.set(true);
    try {
      const res = await this.auth.listUsers(pageToken, this.pageSize());
      this.users.set(res.users);
      this.currentPageToken.set(pageToken);
      this.nextPageToken.set(res.nextPageToken || '');
      this.hasMore.set(!!res.nextPageToken);
    } catch (err) {
  ...
  nextPage() {
    ...
    this.tokenStack.update((tokens) => [...tokens, token]);
    this.currentPageToken.set(token);
    this.loadUsers(token);
  }

  prevPage() {
    const tokens = this.tokenStack();
    if (tokens.length <= 1) return;

    const prevTokens = tokens.slice(0, -1);
    const prevToken = prevTokens[prevTokens.length - 1];

    this.tokenStack.set(prevTokens);
    this.page.update((p) => p - 1);
    this.currentPageToken.set(prevToken);
