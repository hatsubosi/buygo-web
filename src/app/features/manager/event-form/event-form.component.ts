import { Component, inject, signal, OnInit, Input, ViewChild , ChangeDetectionStrategy } from '@angular/core';
...
@Component({
    ...
    imports: [ReactiveFormsModule, RouterLink, UiContainerComponent, UiBtnComponent, UiDialogComponent, ManagerSelectorComponent],
...
export class EventFormComponent implements OnInit {
    ...
    createDiscountGroup(discount?: any): FormGroup {
        return this.fb.group({
            ...
            discountAmount: [Number(discount?.discountAmount) || 0, [Validators.required, Validators.min(0)]]
        });
    }
    ...
    async onSubmit() {
        ...
        try {
            if (this.isEditMode() && this.eventId()) {
                ...
                await this.eventService.createEvent(val.title, val.description, start, end, val.items, val.discounts);
