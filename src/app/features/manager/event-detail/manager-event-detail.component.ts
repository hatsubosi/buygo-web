import { Component, inject, computed, input, effect, signal, ViewChild, ChangeDetectionStrategy } from '@angular/core';
...
@Component({
    ...
    imports: [RouterLink, UiContainerComponent, UiBtnComponent, DatePipe, CurrencyPipe, SlicePipe, FormsModule, UiDialogComponent],
    templateUrl: './manager-event-detail.component.html',
    styleUrl: './manager-event-detail.component.css'
})
export class ManagerEventDetailComponent { // Implements OnInit removed to use signal effect logic if desired, or restore it.
    ...
    async changeEventStatus(status: number) {
        ...
        let action = '';
        switch (status) {
            case 2: action = 'publish'; break;
            case 3: action = 'end'; break;
            case 4: action = 'archive'; break;
            default: return;
        }
